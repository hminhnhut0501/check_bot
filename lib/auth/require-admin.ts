...
export async function requireReviewer(request: Request) {
  ...
  const permission = await requirePermission(request, 'group_bot.read');
...
export async function requirePermission(request: Request, permission: string) {
  ...
  const allowed = (roles ?? []).some((entry: { roles: unknown }) => {
    const role = entry.roles as { name: string; permissions: Record<string, boolean> };
