...
create or replace function public.set_group_bot_updated_at()
returns trigger
language plpgsql
as $$
begin
...
create trigger bot_groups_updated_at
before update on public.bot_groups
for each row execute function public.set_group_bot_updated_at();

drop trigger if exists bot_group_settings_updated_at on public.bot_group_settings;
create trigger bot_group_settings_updated_at
before update on public.bot_group_settings
for each row execute function public.set_group_bot_updated_at();

drop trigger if exists bot_members_updated_at on public.bot_members;
create trigger bot_members_updated_at
before update on public.bot_members
for each row execute function public.set_group_bot_updated_at();

drop trigger if exists bot_blacklist_items_updated_at on public.bot_blacklist_items;
create trigger bot_blacklist_items_updated_at
before update on public.bot_blacklist_items
for each row execute function public.set_group_bot_updated_at();

drop trigger if exists bot_welcome_messages_updated_at on public.bot_welcome_messages;
create trigger bot_welcome_messages_updated_at
before update on public.bot_welcome_messages
for each row execute function public.set_group_bot_updated_at();

drop trigger if exists bot_moderation_rules_updated_at on public.bot_moderation_rules;
create trigger bot_moderation_rules_updated_at
before update on public.bot_moderation_rules
for each row execute function public.set_group_bot_updated_at();
...
select
  id, true, true, false, true, true, false, '{"mode":"standard","seed":"standalone"}'::jsonb
from seeded_group
